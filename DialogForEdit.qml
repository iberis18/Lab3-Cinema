...
Window {
    ...
    title: qsTr("Редактирование информации о фильме")
    minimumWidth: 400
    maximumWidth: 400
    minimumHeight: 200
    maximumHeight: 200
    ...
    Button {
        visible: {textName.length>0 && textGenre.length>0 && root.currentIndex <0}
    ...
    Button {
        ...
        onClicked: {
            root.hide()
            edit(textName.text, textGenre.text, textDirector.text, textYear.text, textAge.text, root.currentIndex)
        }
    }
    Button {
        id: buttonCancel
    ...
    function execute(name, genre, director, year, age, index){
        textName.text = name
